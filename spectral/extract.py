...
def extract_features(
    ...
    for i, (images, files, indices) in enumerate(pbar):
        ...
        if 'dino' in model_name or 'mocov3' in model_name:
            ...
            images = images.to(accelerator.device)
            features = utils.extract_feat_res(images, model, feat_ids, bottleneck_ids, lids)
            feat = features[26]
            b, c, h, w = feat.size()
            output_dict['k'] = feat.view(b, c, -1).permute(0, 2, 1)
        elif 'resnet50' in model_name:
            ...
            feat_ids = [17, 19, 21, 24, 26, 28, 30]
            nbottlenecks = [2, 2, 3, 3, 3, 1]
            bottleneck_ids = reduce(add, list(map(lambda x: list(range(x)), nbottlenecks)))
            lids = reduce(add, [[i + 1] * x for i, x in enumerate(nbottlenecks)])
            images = images.to(accelerator.device)
            features = utils.extract_feat_vgg(images, model, feat_ids, bottleneck_ids, lids)
            feat = features[5]
            b, c, h, w = feat.size()
            output_dict['k'] = feat.view(b, c, -1).permute(0, 2, 1)
        else:
...
def _extract_eig(
    ...
    image_downsample_factor: Optional[int] = 8,
    ...
    # Eigenvectors of matting laplacian matrix
        ...
        if image_color_lambda > 0:
            ...
            image_file = str(Path(images_root) / data_dict['file'])
...
def _extract_multi_region_segmentations(
    ...
    # Sizes
    B, C, H, W, P, H_patch, W_patch, H_pad, W_pad = utils.get_image_sizes(data_dict, 4)
